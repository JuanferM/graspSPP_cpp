...
std::tuple<int, int, std::unique_ptr<int[]>, std::unique_ptr<char[]>, std::unique_ptr<float[]>> loadSPP(std::string fname)
{
  ...
  try {
    if(f.is_open()) {
      ...
      C = std::make_unique<int[]>(n), U = std::make_unique<float[]>(n), A = std::make_unique<char[]>(n*m);
